...
def scrape_page(page_num, config, update_global=True):
    ...
    for item in items:
        ...
        thumbnail = urljoin(base_url, thumbnail_div.find('img').get('src', 'N/A')) if thumbnail_div and thumbnail_div.find('img') else 'N/A'
...
def save_data_txt(config):
    try:
        ...
        columns = ['page', 'id', 'code', 'link', 'thumbnail', 'ribbons', 'ribboni', 'categories', 'comment_count', 'meta_description']
