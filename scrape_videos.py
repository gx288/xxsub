...
def scrape_page(page_num, config, update_global=True):
    ...
    for item in items:
        ...
        thumbnail = urljoin(base_url,
                            thumbnail_div.find('img').get('src', 'N/A')) if thumbnail_div and thumbnail_div.find(
            'img') else 'N/A'
...
def save_data_txt(config):
    try:
        ...
        # Convert id to numeric for sorting only
        df['id_numeric'] = pd.to_numeric(df['id'], errors='coerce')
        df = df.drop_duplicates(subset=['id', 'link'], keep='last')
        df = df.sort_values(by=['page', 'id_numeric'], ascending=[True, False])
...
def update_google_sheets(config):
    ...
    try:
        ...
        if all_video_data:
            ...
            for row in values[1:]:
                if len(row) > 1:
                    row[1] = str(row[1])  # id is second column
